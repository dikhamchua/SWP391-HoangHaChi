...
| Script | Purpose | Refs |
|---|---|---|
| `check-secrets.sh` | Block staged credentials (web.xml db params, hardcoded `DatabaseUtil` constants, `.env` files) | HRS-001 |
| `check-jsp-xss.sh` | Block JSP XSS smell — pagination baseUrl built from raw EL, `<input value="${...}">` without `<c:out>` wrap | HARNESS_BACKLOG #6, #11 |

Both are chained from `.git/hooks/pre-commit`. Run manually with:
...
`check-jsp-xss.sh` whitelists numeric PK form fields (`value="${entity.id}"` and `value="${entity.somethingId}"`) — they are integers from JDBC, not user input.
