-- ============================================
-- HỆ THỐNG QUẢN LÝ CHUỖI CỬA HÀNG - SQL SERVER SCHEMA
...
-- Bảng Stores (Cửa hàng/Chi nhánh)
...
-- Thêm foreign key cho Stores.ManagerID
...
-- Trigger để tự động update ModifiedDate cho Users
...
-- Trigger để tự động update ModifiedDate cho Stores
...
-- Bảng RolePermissions (Phân quyền)
...
PRINT 'Part 1: Quản lý người dùng & phân quyền - COMPLETED';
GO
...
-- Trigger để tự động update ModifiedDate cho Products
...
-- Bảng PriceBookDetails (Chi tiết bảng giá)
...
-- Bảng PriceBookStores (Bảng giá theo chi nhánh)
...
-- Bảng PriceBookCustomerGroups (Bảng giá theo nhóm khách hàng)
...
-- Bảng DiscountConfigs (Cấu hình giảm giá)
...
PRINT 'Part 2: Quản lý sản phẩm - COMPLETED';
GO
...
-- Trigger để tự động update LastUpdated cho Inventories
...
-- Bảng StockTransfers (Chuyển kho)
...
-- Bảng StockTransferDetails (Chi tiết chuyển kho)
...
-- Bảng StockCheckSessions (Phiên kiểm kho)
...
-- Trigger cho StockCheckSessions
...
-- Bảng StockCheckDetails (Chi tiết kiểm kho)
...
-- Bảng StockCheckParticipants (Người tham gia kiểm kho)
...
-- Bảng StockAdjustments (Điều chỉnh tồn kho)
...
PRINT 'Part 3: Quản lý kho - COMPLETED';
GO
...
-- Bảng CustomerTiers (Hạng thành viên)
...
-- Bảng LoyaltyTransactions (Lịch sử tích điểm)
...
PRINT 'Part 4: Quản lý khách hàng - COMPLETED';
GO
...
-- Thêm foreign key cho LoyaltyTransactions.InvoiceID
...
-- Bảng OrderReturns (Đơn trả hàng)
...
-- Bảng OrderReturnDetails (Chi tiết trả hàng)
...
PRINT 'Part 5: Quản lý bán hàng - COMPLETED';
GO

-- ============================================
-- HỆ THỐNG QUẢN LÝ CHUỖI CỬA HÀNG - SQL SERVER SCHEMA
-- Phần 6: Quản lý nhà cung cấp & nhập hàng
-- ============================================
...
-- ============================================
-- 6. QUẢN LÝ NHÀ CUNG CẤP & NHẬP HÀNG
-- ============================================
...
-- Bảng PurchaseOrders (Đơn đặt hàng)
...
-- Bảng PurchaseOrderDetails (Chi tiết đơn đặt hàng)
...
-- Bảng SupplierPayments (Thanh toán nhà cung cấp)
...
-- Bảng SupplierDebts (Công nợ nhà cung cấp)
...
PRINT 'Part 6: Quản lý nhà cung cấp & nhập hàng - COMPLETED';
GO

-- ============================================
-- HỆ THỐNG QUẢN LÝ CHUỖI CỬA HÀNG - SQL SERVER SCHEMA
-- Phần 7: Quản lý thu chi, báo cáo, website & cấu hình
-- ============================================
...
-- Bảng SalesReports (Báo cáo doanh số)
...
-- Bảng LoyaltyReports (Báo cáo khách hàng thân thiết)
...
-- Bảng InventoryReports (Báo cáo tồn kho)
...
-- Bảng ExportLogs (Lịch sử export báo cáo)
...
-- Bảng WebsiteContents (Nội dung website)
...
-- Bảng ContactForms (Form liên hệ)
...
PRINT 'Part 7: Quản lý thu chi, báo cáo, website & cấu hình - COMPLETED';
GO

-- ============================================
-- HỆ THỐNG QUẢN LÝ CHUỖI CỬA HÀNG - SQL SERVER SCHEMA
-- Phần 8: Dữ liệu mẫu (Sample Data)
-- ============================================
...
BEGIN
    ...
    ('Owner', N'Chủ chuỗi cửa hàng'),
    ('StoreManager', N'Quản lý cửa hàng'),
    ('SalesStaff', N'Nhân viên bán hàng'),
    ('WarehouseStaff', N'Nhân viên kho');
...
BEGIN
    ...
    ('Product', N'Quản lý sản phẩm', 1),
    ('Sales', N'Quản lý bán hàng', 2),
    ('Inventory', N'Quản lý kho', 3),
    ('Customer', N'Quản lý khách hàng', 4),
    ('Supplier', N'Quản lý nhà cung cấp', 5),
    ('Report', N'Báo cáo', 6),
    ('System', N'Quản lý hệ thống', 7);
...
BEGIN
    ...
    ('Normal', 0, 4999999, 0, N'Không có ưu đãi đặc biệt'),
    ('Silver', 5000000, 19999999, 3, N'Ưu tiên đổi trả'),
    ('Gold', 20000000, 49999999, 5, N'Ưu tiên đổi trả + quà sinh nhật'),
    ('VIP', 50000000, NULL, 10, N'Tất cả ưu đãi + mua nợ');
    PRINT 'Inserted CustomerTiers';
END
GO

-- 6. TransactionTypes - Income
IF NOT EXISTS (SELECT * FROM TransactionTypes WHERE TypeName = N'Thu bán hàng')
BEGIN
    INSERT INTO TransactionTypes (TypeName, Category, Description) VALUES
    (N'Thu bán hàng', 'Income', N'Doanh thu từ bán sản phẩm'),
    (N'Thu công nợ khách hàng', 'Income', N'Khách hàng thanh toán công nợ');
    PRINT 'Inserted TransactionTypes - Income';
END
GO

-- 7. TransactionTypes - Expense
IF NOT EXISTS (SELECT * FROM TransactionTypes WHERE TypeName = N'Chi nhập hàng')
BEGIN
    INSERT INTO TransactionTypes (TypeName, Category, Description) VALUES
    (N'Chi nhập hàng', 'Expense', N'Nhập sản phẩm từ nhà cung cấp'),
    (N'Chi lương nhân viên', 'Expense', N'Trả lương/thưởng cho nhân viên'),
    (N'Chi vận chuyển', 'Expense', N'Chi phí vận chuyển hàng hóa'),
    (N'Chi marketing', 'Expense', N'Chi phí quảng cáo, khuyến mãi'),
    (N'Chi hoàn tiền', 'Expense', N'Refund khách hàng'),
    (N'Chi vận hành', 'Expense', N'Điện, nước, internet, mặt bằng'),
    (N'Chi khác', 'Expense', N'Các khoản chi khác');
...
BEGIN
    ...
    ('return_days', '7', 'Number', 'Return', N'Số ngày được đổi trả hàng'),
    ('loyalty_points_rate', '1000', 'Number', 'Loyalty', N'1000đ = 1 điểm'),
    ('vat_percent', '10', 'Number', 'Tax', N'Thuế VAT %'),
    ('tier_silver_min', '5000000', 'Number', 'Loyalty', N'Ngưỡng tối thiểu hạng Silver'),
    ('tier_gold_min', '20000000', 'Number', 'Loyalty', N'Ngưỡng tối thiểu hạng Gold'),
    ('tier_vip_min', '50000000', 'Number', 'Loyalty', N'Ngưỡng tối thiểu hạng VIP'),
    ('allow_negative_inventory', 'false', 'Boolean', 'General', N'Cho phép bán âm kho'),
    ('company_name', N'Chuỗi Cửa Hàng ABC', 'String', 'General', N'Tên công ty'),
    ('company_phone', '0123456789', 'String', 'General', N'Số điện thoại công ty'),
    ('company_email', 'contact@company.com', 'String', 'General', N'Email công ty'),
    ('company_address', N'123 Đường ABC, Quận 1, TP.HCM', 'String', 'General', N'Địa chỉ công ty');
    PRINT 'Inserted SystemSettings';
END
GO

-- ============================================
-- TẠO PERMISSIONS TỰ ĐỘNG
-- ============================================

-- Tạo tất cả permissions từ Modules và Actions
...
-- ============================================
-- PHÂN QUYỀN CHO ADMIN (TOÀN QUYỀN)
-- ============================================
...
PRINT '1. Tạo user Admin đầu tiên';
PRINT '2. Tạo Stores (cửa hàng)';
PRINT '3. Tạo Warehouses (kho)';
PRINT '4. Tạo Categories và Products';
PRINT '5. Phân quyền cho các Role khác (Owner, StoreManager, SalesStaff, WarehouseStaff)';
PRINT '';
PRINT 'Ví dụ tạo user Admin:';
...
PRINT '';
PRINT 'Lưu ý: Password cần được mã hóa bằng bcrypt hoặc tương tự';
PRINT '============================================';
GO
