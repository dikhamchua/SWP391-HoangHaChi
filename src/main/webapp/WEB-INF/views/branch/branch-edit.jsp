...
<fmt:formatDate value="${branch.createdAt}" pattern="dd/MM/yyyy HH:mm" var="createdAtText" />
    ...
    <div class="kr-main">
        ...
        <div class="kr-content" style="border:none; background:transparent; overflow:visible;">
            ...
            <style>
                ...
                #branch-tab-general:checked ~ .kr-tab-card .kr-tab-general,
                #branch-tab-history:checked ~ .kr-tab-card .kr-tab-history { display:block; }
                #branch-tab-general:checked ~ .kr-tab-card label[for="branch-tab-general"],
                #branch-tab-history:checked ~ .kr-tab-card label[for="branch-tab-history"] {
            ...
            <form method="post" action="${ctx}/admin/branches" autocomplete="off">
                ...
                <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:16px; margin-bottom:16px; flex-wrap:wrap;">
                    <div>
                        ...
                        <h1 class="kr-page-title" style="margin:0;">Chi tiết chi nhánh</h1>
                    </div>
                    <div style="display:flex; align-items:center; gap:8px; flex-shrink:0;">
                        <button type="submit" class="kr-btn kr-btn-primary">Lưu</button>
                        <a class="kr-btn" href="${ctx}/admin/branches">Quay lại</a>
                    </div>
                </div>

                <input class="kr-tab-radio" type="radio" id="branch-tab-general" name="branchTab" checked />
                <input class="kr-tab-radio" type="radio" id="branch-tab-history" name="branchTab" />
                ...
                <div class="kr-tab-card" style="background:#fff; border:1px solid #e8eaed; border-radius:10px; overflow:hidden;">
                    <div style="display:flex; align-items:flex-end; gap:8px; padding:0 24px; border-bottom:1px solid #e8eaed; background:#f8fafc;">
                        <label for="branch-tab-general" style="font-size:15px; font-weight:700; color:#5f6368; padding:14px 18px; margin-top:12px; margin-bottom:-1px; background:#f8fafc; border:1px solid #e8eaed; border-radius:8px 8px 0 0; cursor:pointer;">Thông tin chung</label>
                        <label for="branch-tab-history" style="font-size:15px; font-weight:700; color:#5f6368; padding:14px 18px; margin-top:12px; margin-bottom:-1px; background:#f8fafc; border:1px solid #e8eaed; border-radius:8px 8px 0 0; cursor:pointer;">Lịch sử hoạt động</label>
                    </div>
                        ...
                        <div style="display:flex; flex-direction:column; gap:24px;">
                            <section style="display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; padding:24px; border:1px solid #eef0f2; border-radius:10px; background:#f8fafc; text-align:center;">
                                <div style="width:88px; height:88px; border-radius:50%; background:#e8f1ff; color:#0070f4; display:flex; align-items:center; justify-content:center; border:3px solid #fff; box-shadow:0 2px 8px rgba(0,0,0,.08);">
                                    <svg style="width:42px;height:42px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                                </div>
                                <div style="font-size:20px; font-weight:700; color:#15171a;"><c:out value="${branch.name}" /></div>
                                <div style="display:flex; align-items:center; justify-content:center; gap:8px; flex-wrap:wrap;">
                                    <span style="font-size:13px; color:#5f6368; background:#fff; border:1px solid #e8eaed; border-radius:999px; padding:5px 12px;">CN${branch.branchId}</span>
                                    <c:choose>
                                        <c:when test="${branch.status == 'active'}"><span class="kr-status active">Hoạt động</span></c:when>
                            ...
                            <section style="display:grid; grid-template-columns:repeat(auto-fit, minmax(260px, 1fr)); gap:20px 16px;">
                                <div style="display:flex; flex-direction:column; gap:8px;">
                                    <label style="font-size:13px; font-weight:600;">Mã chi nhánh</label>
                                    <input type="text" value="CN${branch.branchId}" readonly style="width:100%; height:38px; padding:0 12px; border:1px solid #e8eaed; border-radius:6px; font-size:14px; background:#f8fafc; box-sizing:border-box;"/>
                                </div>
                                <div style="display:flex; flex-direction:column; gap:8px;">
                                    <label style="font-size:13px; font-weight:600;">Tên chi nhánh <span style="color:#ef4444;">*</span></label>
                                    <input type="text" name="name" required maxlength="255" value="<c:out value='${branch.name}'/>" style="width:100%; height:38px; padding:0 12px; border:1px solid #e8eaed; border-radius:6px; font-size:14px; box-sizing:border-box;"/>
                                </div>
                                <div style="display:flex; flex-direction:column; gap:8px;">
                                    <label style="font-size:13px; font-weight:600;">Số điện thoại</label>
                                    <input type="text" name="phone" maxlength="30" value="<c:out value='${branch.phone}'/>" style="width:100%; height:38px; padding:0 12px; border:1px solid #e8eaed; border-radius:6px; font-size:14px; box-sizing:border-box;"/>
                                </div>
                                <div style="display:flex; flex-direction:column; gap:8px;">
                                ...
                                <div style="display:flex; flex-direction:column; gap:8px; grid-column:1/-1;">
                                    ...
                                    <input type="text" name="address" maxlength="255" value="<c:out value='${branch.address}'/>" style="width:100%; height:38px; padding:0 12px; border:1px solid #e8eaed; border-radius:6px; font-size:14px; box-sizing:border-box;"/>
                                </div>
                            </section>
                        </div>
                    </div>
